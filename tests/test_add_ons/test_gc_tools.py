...
class Test_gc_var_or_callable_parameter(unittest.TestCase):
	...
	def test_multiline_default(self):
		result = self.test_parameter(
			strarg=add_ons.gc_tools.gc_var_or_callable_parameter("@STRARG@", callable=self.test_parameter),
			intarg=add_ons.gc_tools.gc_var_or_callable_parameter("@INTARG@", callable=self.test_parameter),
			fltarg=add_ons.gc_tools.gc_var_or_callable_parameter("@FLTARG@", callable=self.test_parameter),
		)
	...
	def test_multiline_overwrite(self):
		result = self.test_parameter(
			strarg=add_ons.gc_tools.gc_var_or_callable_parameter('"foo"', callable=self.test_parameter),
			intarg=add_ons.gc_tools.gc_var_or_callable_parameter("321", callable=self.test_parameter),
			fltarg=add_ons.gc_tools.gc_var_or_callable_parameter("1.0", callable=self.test_parameter),
		)
	...
	def test_singleline(self):
		result = self.test_parameter(strarg="bar", intarg=add_ons.gc_tools.gc_var_or_callable_parameter("321", callable=self.test_parameter), fltarg=3.0)
	...
	def test_renamed(self):
		from add_ons.gc_tools import gc_var_or_callable_parameter as func_alias
